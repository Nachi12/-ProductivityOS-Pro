...
// Ignore Chrome DevTools workspace requests to prevent CSP error logs
    ...
    try {
        ...
        if (profile && profile.familyId) {
            ...
            await Family.findOneAndUpdate(
                ...
                { sharedData: safeSharedData, lastUpdated: new Date() }
            );
        }
...
app.post('/api/family/accept-invite', async (req, res) => {
    try {
        ...
        if (linkIdToken && linkIdToken !== 'google-link-token') {
            ...
            const authHeader = req.headers.authorization;
            const headerUid = req.headers['x-user-uid'];
            verifiedTarget = await verifyToken(authHeader, headerUid);
        }
        
        if (!verifiedTarget || !verifiedTarget.uid) {
            return res.status(401).json({ error: 'Valid authentication is strictly required to accept invitations.' });
        }
...
app.post('/api/family/link-account', requireAuth, async (req, res) => {
    try {
        ...
        if (!verifiedTarget || !verifiedTarget.uid) {
            return res.status(401).json({ error: 'Invalid Google account authentication token for linking.' });
        }
...
app.post('/api/statement/parse', requireAuth, (req, res) => {
    try {
